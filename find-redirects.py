...
def writeObjectKeys(bucket)-> list:
    ...
    f = open(f"{bucket}-keys.txt", 'a')
    f.write(str(objects))
    return objects

def getObjectKeys(bucket)-> list:
    f = open(f"{bucket}-keys.txt", 'r')
    objects = list(f.read())
    return objects
...
def main() -> None:
    ...
    objects = writeObjectKeys(bucket) if not args.skipKeyGeneration else getObjectKeys(bucket)


    s3_connection = boto3.session.Session().client("s3")
    redirect_file_num = '-{args.redirectFileNumber}' if args.redirectFileNumber else ""
    f = open(f"{args.bucket}-redirects{redirect_file_num}.txt", 'a')
